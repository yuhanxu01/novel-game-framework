...
# 自动化配置
MAX_RUN_TIME=1800          # 单次最大运行时间（30分钟）
...
EOF
    ...
    # 使用 opencode run 传递指令
    if command -v timeout &> /dev/null; then
        timeout $MAX_RUN_TIME opencode run "$(cat AUTO_INSTRUCTION.txt)" >> "$LOG_DIR/opencode_output.log" 2>&1 &
    else
        # macOS 没有 timeout，使用后台运行 + 手动超时控制
        opencode run "$(cat AUTO_INSTRUCTION.txt)" >> "$LOG_DIR/opencode_output.log" 2>&1 &
    fi
    ...
    # 监控进程
    if monitor_process $opencode_pid; then
